...
EDITABLE_KEYS: dict[str, tuple[str, str, type, str, bool]] = {
    ...
    "dip.price_change_window_seconds":      ("dip", "price_change_window_seconds",      float, "比較ウィンドウ開始 (x秒前, 0=無効)",          False),
    "dip.price_change_window_end_seconds":  ("dip", "price_change_window_end_seconds",  float, "比較ウィンドウ終端 (y秒前, 0=現在)",           False),
    "dip.price_change_min_rate":            ("dip", "price_change_min_rate",            float, "変動率閾値 (%, x〜y秒前の平均と現在価格を比較)", True),
    "tracking.poll_interval":      ("tracking", "poll_interval",      float, "急変動期ポーリング間隔 (秒)",         False),
    "tracking.poll_interval_slow": ("tracking", "poll_interval_slow", float, "安定期ポーリング間隔 (秒, 0=常に高速)", False),
